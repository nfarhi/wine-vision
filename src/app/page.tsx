...
function ResultCard({ data }: { data: ApiResult }) { const label = data.recognizedLabel ?? {}; const price = data.priceEstimate ?? {}; return <article className="rounded-3xl border border-[#ded8cd] bg-white p-6 shadow-sm"><div className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-wider text-[#9b5b3d]">Latest reading</p><h2 className="mt-2 font-serif text-3xl">{label.wine || "Unidentified wine"}</h2><p className="mt-1 text-sm text-[#716d64]">{label.producer || "Producer unknown"}{label.vintage ? ` · ${label.vintage}` : ""}</p></div><span className="rounded-full bg-[#f3e5dc] px-3 py-1 text-xs text-[#9b5b3d]">Analyzed</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Region", label.region || label.country || "—"],["Price", price.low != null && price.high != null ? `${price.currency || "£"}${price.low}–${price.high}` : "—"],["Confidence", price.confidence || "—"],["Drink window", data.drinkWindow?.from || "—"]].map(([key, value]) => <div key={key} className="rounded-2xl bg-[#f7f5f0] p-3"><p className="text-[10px] uppercase tracking-wider text-[#9b958b]">{key}</p><p className="mt-2 text-sm font-medium">{value}</p></div>)}</div>{price.note && <p className="mt-5 text-xs leading-5 text-[#898277]">{price.note}</p>}</article>; }
